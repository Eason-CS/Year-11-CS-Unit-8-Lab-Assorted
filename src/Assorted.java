...
public class Assorted {
    ...
    public static List<String> lineNumbering(List<String> list) {
        ArrayList<String> Prepended = new ArrayList<>();

        for (int i = 0; i < list.size(); i++) {

            int num = i + 1;
            String temp = num + ": " + list.get(i);
            Prepended.add(temp);
        }
        return Prepended;
    }
    ...
    public static int busStop(List<Integer[]> list) {
        ...
        for (Integer[] pair : list) {
            remaining = remaining + pair[0] - pair[1];
        }
        return remaining;
    }
    ...
    public static int toBinary(List<Integer> list) {
        ...
        for (int i = list.size() - 1; i >= 0; i--) {
            if (list.get(i) == 1) {
                binary += (int) Math.pow(2, list.size() - 1 - i); // Reverse order
            }
        }
        return binary;
    }
    ...
    public static List<Integer> sortOdd(List<Integer> list) {
        ...
        List<Integer> alt = new ArrayList<>(list);
    ...
    public static List<Integer> uniqueNumber(int lowerBound, int upperBound) {
        ...
        for (int i = lowerBound; i <= upperBound; i++) {
            ...
            for (int a = 1; a <= digits; a++) {
                int digit = Character.getNumericValue(num.charAt(a - 1));
                checker += (int) Math.pow(digit, a);
            }
            if (i == checker) {
                result.add(i);
            }
    ...
    public static List<Integer> filterNTimes(List<Integer> list, int n) {
        ...
        ArrayList<Integer> unique = new ArrayList<>(); // to store every unique motif we find

        for (int i : list) {
            if (!unique.contains(i)) {
                unique.add(i); // creating list of unique motifs
            }
        }
        for (int a : list) {
            int count = 0;
    ...
    public static List<String> wildWest(List<String> directions) {
        ...
        for (String direction : directions) {
            ...
            if (simplified.isEmpty() || !pairs(direction, simplified.get(simplified.size() - 1))) {
    ...
    private static boolean pairs(String d1, String d2) {
        return (d1.equals("NORTH") && d2.equals("SOUTH")) ||
                (d1.equals("WEST") && d2.equals("EAST")) ||
                (d1.equals("SOUTH") && d2.equals("NORTH")) ||
                (d1.equals("EAST") && d2.equals("WEST"));
    }
    ...
    public static int queueTime(List<Integer> queue, int tillsOpen) {
        ...
        int result = tills.get(tills.size() - 1);
        return result;
    }
}